...
export function ProductListSection() {
  ...
  function searchArtist(artist = "") {
    ...
    const fetchData = async () => {
      axios
        .get("https://pjpw.vercel.app/listar" + artist)
        .then((response) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      axios
        .get("https://pjpw.vercel.app/listar")
        .then((response) => {
          if (response.status === 200) {
            setListing(response.data);
          } else {
  ...
  return (
    <section>
              ...
              <div className="p-4 border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                <div className="flex items-start justify-between p-5 border-b border-solid border-blueGray-200 rounded-t">
                  <h3 className="text-3xl font-semibold">
                    Selecione seus artistas preferidos!
                  </h3>
                  <button
                  ...
                  <ul className="max-w-md divide-y divide-gray-200 dark:divide-gray-700">
                    <li className="pb-3 sm:pb-4">
                      <div className="flex items-center space-x-4 rtl:space-x-reverse">
                        ...
                        <NextLink
                          ...
                          onClick={() =>
                            searchArtist("?filtro=Vintage Culture")
                          }
                    ...
                    <li className="py-3 sm:py-4">
                      <div className="flex items-center space-x-4 rtl:space-x-reverse">
                        ...
                        <NextLink
                          onClick={() => searchArtist("?filtro=Mochakk")}
                          href="/products?artista=Mochakk"
                          className="flex-1 min-w-0"
                        >
                          <p className="text-sm font-medium text-gray-900 truncate">
                            Mochakk
                    ...
                    <li className="py-3 sm:py-4">
                      <div className="flex items-center space-x-4 rtl:space-x-reverse">
                        ...
                        <NextLink
                          onClick={() => searchArtist("?filtro=Veigh")}
                          href="/products?artista=Veigh"
                          className="flex-1 min-w-0"
                        >
                          <p className="text-sm font-medium text-gray-900 truncate">
                            Veigh
                    ...
                    <li className="py-3 sm:py-4">
                      <div className="flex items-center space-x-4 rtl:space-x-reverse">
                        ...
                        <NextLink
                          onClick={() => searchArtist("?filtro=Anitta")}
                          href="/products?artista=Anitta"
                          className="flex-1 min-w-0"
                        >
                          <p className="text-sm font-medium text-gray-900 truncate">
                            Anitta
      ...
      <div className="mb-10 mt-24 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 xl:gap-x-8">
        {Listing.map(
          ({ id, title, datetime, venue, photo, description }: any) => (
            <article
              key={id}
              className="cursor-pointer relative overflow-hidden rounded-lg shadow transition hover:shadow-lg"
            >
              <a
                onClick={() =>
                  handleEvent(id, title, datetime, venue, photo, description)
                }
              >
                <img
                  alt="Office"
